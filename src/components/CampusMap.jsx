...
export default function CampusMap({
  buildings,
  selectedBuilding,
  onSelectBuilding,   // <-- SCRUM-19: called on marker click
  route,
  accessibilityMode,
  showTransit,
}) {
        ...
        {route && route.length > 1 && (
          <>
            ...
            <circle cx={route[route.length-1].x} cy={route[route.length-1].y} r="7"
              fill={accessibilityMode ? '#16a34a' : '#2563eb'} stroke="#fff" strokeWidth="2" />
          </>
        )}

        {/* Building markers — SCRUM-19: onClick triggers BuildingPanel */}
        {buildings.map(b => {
          const isSelected = selectedBuilding?.id === b.id
          const acc = b.accessibility
          const hasAccessIssue = accessibilityMode && (!acc.elevator || !acc.ramp)

          return (
            <g
              key={b.id}
              style={{ cursor: 'pointer' }}
              onClick={() => onSelectBuilding(b)}   // ← SCRUM-19
            >
              <rect
              ...
              />
              {/* Accessibility dots — added in SCRUM-21 */}
